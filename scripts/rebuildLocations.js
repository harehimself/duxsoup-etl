...
/**
 * Rebuild Locations Collection from Observations
 *
 * Admin script to rebuild the locations collection from visits/scans observations.
 * Idempotent and replayable - safe to run multiple times.
 *
 * Usage:
 *   node scripts/rebuildLocations.js [options]
 *
 * Options:
 ...
 *
 * Examples:
 *   node scripts/rebuildLocations.js --from=2024-01-01
 *   node scripts/rebuildLocations.js --limit=100 --dry-run
 *   node scripts/rebuildLocations.js --source=visit
 */
...
async function rebuildLocations(args) {
  console.log('📍 Starting locations rebuild...', args);
  ...
  try {
    ...
    if (!args.source || args.source === 'visit') {
      ...
      for (let visit = await visitsCursor.next(); visit != null; visit = await visitsCursor.next()) {
        try {
          if (args['dry-run']) {
            console.log(`  [DRY RUN] Would upsert location from visit ${visit._id}`);
            stats.skipped++;
          } else {
            const result = await upsertLocationFromObservation(visit, 'visit');
            if (result) {
    ...
    if (!args.source || args.source === 'scan') {
      ...
      for (let scan = await scansCursor.next(); scan != null; scan = await scansCursor.next()) {
        try {
          if (args['dry-run']) {
            console.log(`  [DRY RUN] Would upsert location from scan ${scan._id}`);
            stats.skipped++;
          } else {
            const result = await upsertLocationFromObservation(scan, 'scan');
            if (result) {
              stats.locations_upserted++;
            } else {
              stats.skipped++;
          ...
          logger.error('Failed to upsert location from scan', {
            scan_id: scan._id,
            error: error.message,
          });
          stats.errors++;
    ...
    console.log(`  Locations upserted: ${stats.locations_upserted}`);
    console.log(`  Errors: ${stats.errors}`);
    console.log(`  Skipped (dry-run/no location): ${stats.skipped}`);
    ...
    logger.error('Rebuild locations failed', {
      error: error.message,
      stack: error.stack,
    });
    process.exit(1);
...
Usage: node scripts/rebuildLocations.js [options]
