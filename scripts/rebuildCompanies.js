...
/**
 * Rebuild Companies Collection from Observations
 *
 * Admin script to rebuild the companies collection from visits/scans observations.
 * Idempotent and replayable - safe to run multiple times.
 *
 * Usage:
 *   node scripts/rebuildCompanies.js [options]
 *
 * Options:
 ...
 *
 * Examples:
 *   node scripts/rebuildCompanies.js --from=2024-01-01
 *   node scripts/rebuildCompanies.js --limit=100 --dry-run
 *   node scripts/rebuildCompanies.js --source=visit
 */
...
async function rebuildCompanies(args) {
  console.log('🏢 Starting companies rebuild...', args);
  ...
  try {
    ...
    if (!args.source || args.source === 'visit') {
      ...
      for (let visit = await visitsCursor.next(); visit != null; visit = await visitsCursor.next()) {
        try {
          if (args['dry-run']) {
            console.log(`  [DRY RUN] Would upsert company from visit ${visit._id}`);
            stats.skipped++;
          } else {
            const result = await upsertCompanyFromObservation(visit, 'visit');
            if (result) {
    ...
    if (!args.source || args.source === 'scan') {
      ...
      for (let scan = await scansCursor.next(); scan != null; scan = await scansCursor.next()) {
        try {
          if (args['dry-run']) {
            console.log(`  [DRY RUN] Would upsert company from scan ${scan._id}`);
            stats.skipped++;
          } else {
            const result = await upsertCompanyFromObservation(scan, 'scan');
            if (result) {
              stats.companies_upserted++;
            } else {
              stats.skipped++;
          ...
          logger.error('Failed to upsert company from scan', {
            scan_id: scan._id,
            error: error.message,
          });
          stats.errors++;
    ...
    console.log(`  Companies upserted: ${stats.companies_upserted}`);
    console.log(`  Errors: ${stats.errors}`);
    console.log(`  Skipped (dry-run/no company): ${stats.skipped}`);
    ...
    logger.error('Rebuild companies failed', {
      error: error.message,
      stack: error.stack,
    });
    process.exit(1);
...
Usage: node scripts/rebuildCompanies.js [options]
