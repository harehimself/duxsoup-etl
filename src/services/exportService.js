...
async function createExportJob(params) {
  ...
  const exportFields = fields && fields.length > 0 ? fields : DEFAULT_CSV_FIELDS;
...
async function processExportJob(jobId) {
  ...
  if (job.status !== 'pending') {
    throw new AppError('INVALID_STATE', `Job ${jobId} is not pending (status: ${job.status})`);
  }
...
async function generateCsv(jobId, data, fields) {
  ...
  logger.info('CSV file generated', { jobId, filePath, rowCount: records.length });
...
async function generateJson(jobId, data) {
  ...
  logger.info('JSON file generated', { jobId, filePath, rowCount: data.length });
...
async function getExportFile(jobId) {
  ...
  try {
    ...
    throw new AppError('FILE_NOT_FOUND', 'Export file has been deleted or expired', 404);
  }
...
function formatLinkedInUrl(id) {
  ...
  if (/^[\w-]{3,100}$/.test(id) && !id.match(/^\d{8,}$/) && !id.match(/^AC[wo]AA/i)) {
