...
- [x] **Fix Dependabot security vulnerabilities across all dependencies** — Resolved all 17 open Dependabot alerts (10 high, 6 medium, 1 low). Production: `express-rate-limit` 8.2.1→8.3.2 (IPv4-mapped IPv6 rate limit bypass), `nodemailer` 8.0.1→8.0.5 (SMTP command injection). Client: `vite` 7.3.1→7.3.2 (file read via WebSocket, path traversal). Dev tooling: `flatted` 3.3.3→3.4.2 (prototype pollution), plus `path-to-regexp`, `minimatch`, `picomatch`, `brace-expansion`, `yaml` patched via `npm audit fix`. 0 vulnerabilities remaining in both root and client.
  - Priority: `high`
  - Category: `security`
  - Impact: Eliminates all known dependency vulnerabilities, most critically the express-rate-limit bypass that could allow attackers to circumvent webhook rate limiting in production.
...
- [x] **Expose webhook processing warnings in API responses and persisted telemetry** — Capacity-limit drops (`MAX_ROLES`/`MAX_EDUCATION`/`MAX_SKILLS`) and soft errors are currently only logged. Return structured `warnings[]` in webhook responses and persist warning counters for visibility in monitoring.
  - Priority: `medium`
  - Category: `feature`
...
- [x] **Expose webhook processing warnings in API responses** — 2026-04-08. Created `src/utils/warningCollector.js` with `createCollector()` factory and 14 `WARNING_CODES`. Threaded optional collector through `personController.upsertFromObservation()`, `updateRolesTimeline()`, `updateEducation()`, `updateSkills()`. Added warnings for: schema violations, no stable ID, invalid email/connections/degree, inverted date ranges, capacity limits (roles/education/skills), company/location/person upsert failures, debounce. Extended `throughputTracker.js` with `warnings`/`warningsByCode` per bucket, surfaced in `/api/health/throughput`. Updated `observationHandler.js` to create collector, capture schema results, attach `warnings[]` to response. Updated `batchWebhookHandler.js` to include per-item warnings. 28 new tests across 3 files. All 1,773 tests pass.
- [x] **Fix Dependabot security vulnerabilities** — 2026-04-08. Resolved all 17 open Dependabot alerts. Production: `express-rate-limit` 8.2.1→8.3.2, `nodemailer` 8.0.1→8.0.5. Client: `vite` 7.3.1→7.3.2. Dev: `flatted` 3.3.3→3.4.2 + audit fix for path-to-regexp, minimatch, picomatch, brace-expansion, yaml. 0 vulnerabilities remaining. All 1,745 tests pass.
- [x] **Strip HTML tags from summary and description fields** — 2026-02-25. Created `src/utils/htmlSanitizer.js` with `stripHtmlTags()` that strips HTML tags via `/<[^>]*>/g` regex and decodes 6 common HTML entities (`&amp;`, `&lt;`, `&gt;`, `&nbsp;`, `&#39;`, `&quot;`). Integrated into `personController.js` as `transform: stripHtmlTags` on summary FIELD_MAPPING entry (summary stays out of SKIP_CLEAN_FIELDS so `cleanString()` still normalizes whitespace after HTML stripping). Applied `cleanString(stripHtmlTags(pos.Description))` to role descriptions in `updateRolesTimeline()` for both existing and new role paths. Created `scripts/backfillHtmlSanitization.js` with --dry-run/--commit/--limit/--batch-size/--target following backfillTwitterNormalization pattern. Added `backfill:html-sanitize` npm script. 29 sanitizer tests + 7 personController tests + 8 backfill tests = 44 new tests. All 1,745 tests pass.
- [x] **Network composition analytics** — 2026-02-25. Completed `GET /api/insights/network-profile` and added `GET /api/insights/network-profile/trends`. Fixed two bugs in the existing service: (1) percentage denominator now uses `totalCount` instead of sum of top-N items, (2) `medianTenureYears` now computes true median via `$push` + client-side `calculateMedian()` instead of duplicate `$avg`. Added `snapshot.degree` index to Person model. Trends endpoint compares current network composition against 30/60/90-day windows using `createdAt`, computing growth rates across 5 dimensions (company, seniority, industry, country, department) via `$facet` aggregation. Generates human-readable insights for significant 30-day growth (>10% AND >2 new). Extracted `buildBaseFilter()` for reuse between profile and trends. Added `TrendWindow` component schema to OpenAPI. 50 new tests (28 service + 22 controller). All 1,701 tests pass.
