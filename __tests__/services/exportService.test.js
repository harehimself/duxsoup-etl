...
describe('ExportService', () => {
  ...
  describe('processExportJob()', () => {
    it('should throw NOT_FOUND when job does not exist', async () => {
      ...
      await expect(processExportJob('nonexistent')).rejects.toThrow('Export job not found');
    });
    ...
    it('should process CSV job and update status to completed', async () => {
      ...
      const result = await processExportJob('job-csv');
    ...
    it('should throw EXPORT_TOO_LARGE when data exceeds max rows', async () => {
      ...
      await expect(processExportJob('job-big')).rejects.toThrow('maximum row limit');
    });
  });
  ...
  describe('getExportJobStatus()', () => {
    ...
    it('should throw NOT_FOUND when job does not exist', async () => {
      ...
      await expect(getExportJobStatus('nonexistent')).rejects.toThrow('Export job not found');
    });
  });
  ...
  describe('getExportFile()', () => {
    ...
    it('should throw NOT_FOUND when job does not exist', async () => {
      ...
      await expect(getExportFile('nonexistent')).rejects.toThrow('Export job not found');
    });
    ...
    it('should throw FILE_NOT_FOUND when filePath is missing', async () => {
      ...
      await expect(getExportFile('job-1')).rejects.toThrow('Export file not found');
    });
    ...
    it('should throw FILE_NOT_FOUND when file no longer exists on disk', async () => {
      ...
      await expect(getExportFile('job-1')).rejects.toThrow('deleted or expired');
    });
  });
});
