...
const { searchPeople, fuzzySearchPeople, smartSearch } = require('../../src/services/searchService');
...
describe('SearchService', () => {
  describe('searchPeople()', () => {
    it('should throw INVALID_QUERY when query is null', async () => {
      await expect(searchPeople({ query: null })).rejects.toThrow('Search query is required');
    });

    it('should throw INVALID_QUERY when query is a number', async () => {
      await expect(searchPeople({ query: 123 })).rejects.toThrow('Search query is required');
    });

    it('should throw INVALID_QUERY when query is empty string', async () => {
      await expect(searchPeople({ query: '   ' })).rejects.toThrow('Search query cannot be empty');
    });
    ...
    it('should return results and metadata for a valid query', async () => {
      const mockDocs = [{ _id: 'person-1', snapshot: { fullName: 'John Doe' } }];
      ...
      expect(queryMock.sort).toHaveBeenCalledWith({ score: { $meta: 'textScore' } });
    ...
    it('should apply field projection when fields param provided', async () => {
      ...
      await searchPeople({ query: 'test', fields: ['snapshot.fullName', 'snapshot.currentTitle'] });
  ...
  describe('fuzzySearchPeople()', () => {
    it('should throw INVALID_QUERY for empty query', async () => {
      await expect(fuzzySearchPeople({ query: '' })).rejects.toThrow('Search query is required');
    });

    it('should throw INVALID_QUERY for null query', async () => {
      await expect(fuzzySearchPeople({ query: null })).rejects.toThrow('Search query is required');
    });
  ...
  describe('smartSearch()', () => {
    ...
    it('should fall back to fuzzy search when text search returns 0', async () => {
      ...
      const fuzzyQueryMock = buildQueryMock([{ _id: 'p1', snapshot: { fullName: 'Jane' } }]);

      Person.find.mockReturnValueOnce(textQueryMock).mockReturnValueOnce(fuzzyQueryMock);
      Person.countDocuments.mockResolvedValue(0);
