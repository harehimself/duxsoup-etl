...
describe('webhookSchemaValidator', () => {
  ...
  describe('validateWebhookSchema()', () => {
    ...
    it('should detect when VisitTime is a number instead of string', () => {
      const payload = validVisitPayload({ VisitTime: 1705312200000 });
      const result = validateWebhookSchema(payload);
      expect(result.valid).toBe(false);
      expect(result.typeErrors).toContainEqual(
        expect.stringContaining('data/VisitTime')
      );
    });
    ...
    it('should never throw even with bizarre input', () => {
      ...
      expect(() => validateWebhookSchema({ type: 'visit', data: null })).not.toThrow();
    });
    ...
    it('should accept CompanyID as integer (DuxSoup sends both string and int)', () => {
      ...
      expect(result.typeErrors.filter(e => e.includes('CompanyID'))).toHaveLength(0);
    });
    ...
    it('should accept My Tags as either array or string', () => {
      ...
      expect(validateWebhookSchema(asArray).typeErrors.filter(e => e.includes('Tags'))).toHaveLength(0);
      expect(validateWebhookSchema(asString).typeErrors.filter(e => e.includes('Tags'))).toHaveLength(0);
    });
  ...
  describe('detectUnknownFields()', () => {
    ...
    it('should detect unknown fields with correct path', () => {
      const result = detectUnknownFields(
        { a: 1, b: 2, c: 3 },
        ['a'],
        'data'
      );
